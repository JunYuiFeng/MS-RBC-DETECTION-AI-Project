<template>
    <div class="bg-white p-10 w-3/4 rounded-3xl">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <div class="flex justify-start"><b>Sample A</b></div>
          <CellStatistics :comparedResultsPage="true" :predictions="comparisons.patient1" />
        </div>
        <div>
          <div class="flex justify-start"><b>Sample B</b></div>
      ...
      <div>
        <ul>
          <li class="flex justify-start text-xs">&#8226; Sample A contains {{ deformedCellsPercentageDifference }}% more deformed cells</li>
          <li class="flex justify-start text-xs">&#8226; Sample B contains {{ healthyCellsPercentageDifference }}% less healthy cells</li>
        </ul>
      </div>
    </div>
  </template>
  ...
  // Reactive references for percentage differences
  ...
  const calculatePercentageDifferences = () => {
    ...
    deformedCellsPercentageDifference.value = Math.round(((patient1.deformedCellsDetected - patient2.deformedCellsDetected) / totalCellsA) * 100);
    healthyCellsPercentageDifference.value = Math.round(((patient1.healthyCellsDetected - patient2.healthyCellsDetected) / totalCellsB) * 100);
  };
    ...
    const dataBar = {
      ...
      datasets: [
        ...
        },
        {
          ...
          backgroundColor: "rgb(164, 101, 241)",
          hoverOffset: 4,
        },
      ],
    };
